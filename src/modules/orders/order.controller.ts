...
export const orderController = {
  ...
  initiateKhaltiPayment: asyncHandler(async (req: Request, res: Response) => {
    const result = await orderService.initiateKhaltiPayment(
      req.params.id,
      req.user!.userId
    );
    res.status(200).json({ success: true, data: result });
  ...
  verifyKhaltiPayment: asyncHandler(async (req: Request, res: Response) => {
    const order = await orderService.verifyKhaltiPayment(req.body.pidx);
    res.status(200).json({ success: true, data: { order } });
  }),
